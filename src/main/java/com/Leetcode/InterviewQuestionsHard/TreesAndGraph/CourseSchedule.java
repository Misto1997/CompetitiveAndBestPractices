...
class CourseSchedule {
    private static class InputReader {
        ...
        public static void main(String[] args) {
            ...
            out.printLine(canFinish(numCourses, prerequisites));
            out.close();
        }
        ...
        private static boolean canFinish(int numCourses, int[][] prerequisites) {
            ...
            return isCyclePresent(graph) == true ? false : true;
        }
        ...
        private static boolean isCyclePresent(Graph graph) {
            ...
            for (int i = 0; i < graph.v; i++) {
                if (checkForCycle(i, graph, visited, stack))
                    return true;
            }
            return false;
        }

        private static boolean checkForCycle(int i, Graph graph, boolean[] visited, boolean[] stack) {
            if (stack[i])
                return true;
            if (visited[i])
                return false;
            ...
            for (Integer nodeIndex : nodesToBeVisited) {
                if (checkForCycle(nodeIndex, graph, visited, stack))
                    return true;
            }
            stack[i] = false;
            return false;
